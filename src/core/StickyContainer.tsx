...
export interface RecyclerChild
  extends React.ReactElement<RecyclerListViewProps> {
  ref: (recyclerRef: any) => {};
  props: RecyclerListViewProps;
}
export default class StickyContainer<
  P extends StickyContainerProps
> extends React.Component<P> {
  ...
  private _stickyHeaderRef: StickyHeader<
    StickyObjectProps,
    StickyObjectState
  > | null = null;
  private _stickyFooterRef: StickyFooter<
    StickyObjectProps,
    StickyObjectState
  > | null = null;
  ...
  constructor(props: P, context?: any) {
    ...
    this._distanceFromWindow = childProps.distanceFromWindow
      ? childProps.distanceFromWindow
      : 0;
  }
  ...
  public render(): JSX.Element {
    this._assertChildType();
    const recycler: ReactElement<RecyclerListViewProps> = React.cloneElement(
      this.props.children,
      {
    ...
    return (
      <View style={this.props.style ? this.props.style : { flex: 1 }}>
        ...
        {this.props.stickyHeaderIndices ? (
          <StickyHeader
            ref={(stickyHeaderRef: any) =>
              this._getStickyHeaderRef(stickyHeaderRef)
            }
        ...
        {this.props.stickyFooterIndices ? (
          <StickyFooter
            ref={(stickyFooterRef: any) =>
              this._getStickyFooterRef(stickyFooterRef)
            }
    ...
    if (this.props.children.ref) {
      if (typeof this.props.children.ref === "function") {
        ...
        throw new CustomError(
          RecyclerListViewExceptions.refNotAsFunctionException
        );
      }
    }
  };
  ...
  private _getStickyHeaderRef = (stickyHeaderRef: any) => {
    if (this._stickyHeaderRef !== stickyHeaderRef) {
      this._stickyHeaderRef = stickyHeaderRef as (StickyHeader<
        StickyObjectProps,
        StickyObjectState
      > | null);
  ...
  private _getStickyFooterRef = (stickyFooterRef: any) => {
    if (this._stickyFooterRef !== stickyFooterRef) {
      this._stickyFooterRef = stickyFooterRef as (StickyFooter<
        StickyObjectProps,
        StickyObjectState
      > | null);
      // TODO: Resetting state once ref is initialized. Can look for better solution.
  ...
  private _onVisibleIndicesChanged = (
    ...
    if (
      this.props.children &&
      this.props.children.props &&
      this.props.children.props.onVisibleIndicesChanged
    ) {
  ...
  private _assertChildType = (): void => {
    if (
      ...
      !this._isChildRecyclerInstance()
    ) {
      throw new CustomError(
        RecyclerListViewExceptions.wrongStickyChildTypeException
      );
    }
  };
