...
export abstract class LayoutManager {
  ...
  // You can ovveride this incase you want to override style in some cases e.g,
  // say you want to enfore width but not height
  ...
  // Return all computed layouts as an array, frequently called, you are
  // expected to return a cached array. Don't compute here.
  public abstract getLayouts(): Layout[];

  // RLV will call this method in case of mismatch with actual rendered
  // dimensions in case of non deterministic rendering You are expected to cache
  // this value and prefer it over estimates provided No need to relayout which
  // RLV will trigger. You should only relayout when relayoutFromIndex is
  // called.
  ...
  public abstract relayoutFromIndex(startIndex: number, itemCount: number):
      void;
}
...
export class WrapGridLayoutManager extends LayoutManager {
  ...
  constructor(
      layoutProvider: LayoutProvider, renderWindowSize: Dimension,
      isHorizontal: boolean = false, cachedLayouts?: Layout[]) {
    super();
    ...
    for (let i = startIndex; i < itemCount; i++) {
      ...
      if (oldLayout && oldLayout.isOverridden) {
        ...
        this._layoutProvider.setComputedLayout(
            this._layoutProvider.getLayoutTypeForIndex(i), itemDim, i);
      }
      ...
      maxBound = this._isHorizontal ? Math.max(maxBound, itemDim.width) :
  ...
  private _checkBounds(
      itemX: number, itemY: number, itemDim: Dimension,
      isHorizontal: boolean): boolean {
    // Disable the grid view mode
    return itemX < 1;
    // return isHorizontal ? (itemY + itemDim.height <= this._window.height) :
